...
void PulseGenerator::begin(){
	Serial.println("[PulseGenerator] Setup is started ...");
	...
	xTaskCreate((TaskFunction_t)PulseGenerator::TaskStart, "MotorDriverTask", 8192, this, 50, &mainTask);

	Serial.println("[PulseGenerator] Setup is done.");
 ...
 */
TaskFunction_t PulseGenerator::TaskStart(void * pvParameters)
{
	PulseGenerator* manager;
	esp_task_wdt_init(MOTOR_DRIVER_WDT_PULSE_GENERATOR_TASK_TIMEOUT, true);
	esp_task_wdt_add(NULL);
	manager = (PulseGenerator*) pvParameters;
	manager->MainTask();
...
void PulseGenerator::MainTask()
{
	...
	{
		Serial.printf("[PulseGenerator] Waiting for zero!\r\n");
		esp_task_wdt_reset();
...
//		Serial.printf("[PulseGenerator] Current: %d, Target: %d\r\n", x_current_half_pulse_counter, x_target_half_pulse_counter);
		PrintStackWatermark();
		vTaskDelay(10/portTICK_PERIOD_MS);
	}
}

void PulseGenerator::PrintStackWatermark()
{
	if(millis() > (stackWatermarkPrintLastMillis + 10000)){
		stackWatermarkPrintLastMillis = millis();
		Serial.printf("[PulseGenerator] Task Stack left: %d\r\n", uxTaskGetStackHighWaterMark(NULL));
		Serial.printf("[PulseGenerator] X Current: %d, Target: %d, freq: %d\r\n", x_current_half_pulse_counter, x_target_half_pulse_counter, x_pulse_frequency);
		Serial.printf("[PulseGenerator] Y Current: %d, Target: %d\r\n", y_current_half_pulse_counter, y_target_half_pulse_counter);
	}
}

bool PulseGenerator::IsReady()
{
	return is_ready;
}

void PulseGenerator::StartPulses()
{
	uint32_t prescaler = 0;
	StopPulses();

	prescaler = MOTOR_DRIVER_ESP_FREQUENCY/8000/x_pulse_frequency/2;
	Serial.printf("[PulseGenerator] Setting the X timer; prescaler: %d\r\n", prescaler);
	...
	Serial.printf("[PulseGenerator] Setting the Y timer; prescaler: %d\r\n", prescaler);
...
void PulseGenerator::StopPulses()
{
...
void PulseGenerator::SetX(float x)
{
...
void PulseGenerator::SetY(float y)
{
...
void PulseGenerator::SetXY(float x, float y)
{
	if(x < MOTOR_DRIVER_X_COARSE)
	{
		x_target_location = x;
 ...
 * speed is mm/s
 */
void PulseGenerator::SetXY(float x, float y, float x_speed, float y_speed)
{
	if(x < MOTOR_DRIVER_X_COARSE)
	{
		x_target_location = x;
...
void PulseGenerator::GetXY(float * x, float * y)
{
